...
- **모든 쓰기는 append-only다.** 과거는 수정되거나 삭제되지 않는다. 사고의 이력은 영구적이다.
...
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `nid: Nid`                     | 뉴런의 고유 식별자.                                                                                                                                            |
| `actor: vector<byte>`          | 이 뉴런을 발화한 주체. 비어 있을 수 있다 — actor가 명시되면 "누가 이 생각을 했는가"를 추적할 수 있다. HDB의 핵심 목적인 사고 주체 추적을 가능하게 하는 필드다. |
| `payload: vector<byte>`        | 사고의 실제 내용. 코어는 형식을 강제하지 않는다.                                                                                                               |
| `timestamp: time_point`        | 사고가 발생한 시각. epoch μs 정밀도. 기억의 시간적 위치를 나타낸다.                                                                                            |
...
| `timestamp: time_point`        | 연결이 확정된 시각. 같은 두 뉴런 사이라도 시점이 다르면 별도의 시냅스다 — append-only 원칙. |
...
| `timestamp: time_point`        | 이 임베딩이 생성된 시각. 같은 뉴런이라도 시간이 지남에 따라 의미가 재해석될 수 있다 — 새 레코드를 삽입해 추상화 이력을 보존한다. |
| `meta: optional<vector<byte>>` | 임베딩 모델 정보 등 부가 정보. 코어는 읽지 않는다.                                                                               |

임베딩 생성은 코어의 책임이 아니다. Python 레이어가 생성한 벡터를 `Thalamus::Consolidate()`로 전달하면 코어는 저장만 한다.
...
2. `start_nid`에서 출발
3. 매 스텝: outgoing edges에 Impulse를 적용 → softmax(temperature) → 확률적 선택 → `Thought`
