...
| 필드                           | 의도                                                                                                                                                                                                                                                  |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `nid: Nid`                     | `timestamp`, `actor`, `payload`, `meta` 네 필드가 함께 이 사고를 정의한다. `nid`는 그 복합 지문을 사람이 다루기 편하게 대표하는 값이다.                                                                                                               |
...
| 필드                           | 의도                                                                                                                                                                                   |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sid: Sid`                     | `timestamp`, `actor`, `from_nid`, `to_nid`, `meta` 가 함께 이 연결을 정의한다. `sid`는 그 복합 지문을 대표하는 값이다.                                                                 |
...
| 필드                           | 의도                                                                                                                                                                                                                 |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `aid: Aid`                     | `timestamp`, `actor`, `nid`, `payload`, `meta` 가 함께 이 추상화를 정의한다. `aid`는 그 복합 지문을 대표하는 값이다.                                                                                                 |
| `actor: vector<byte>`          | 이 추상화를 생성한 주체. 같은 뉴런에서 비롯된 추상화라도 만든 주체가 다르면 다른 관점이다 — 같은 기억을 어떻게 추상화하는지는 누가 바라보는지에 따라 달라진다.                                                       |
| `nid: Nid`                     | 이 추상화가 어떤 뉴런에서 비롯됐는지. 추상화는 반드시 실제 사고에 뿌리를 두어야 한다.                                                                                                                                |
...
```
Fire(actor, from_nid, to_nid, meta?) → optional<Sid>
```
...
**Imagine**(상상하다): Engram 위에서 확률적 그래프 워크를 수행한다. `params`는 `ImagineParams { start_nid, epochs, creativity }`로 전달한다.
