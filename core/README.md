...
0
remizio 준태 웰시코기 메시 우왁굳 강철 FE
"심장을 불태워라" 이준성 이시우 오정우 안 공장 돼 되
어쩔티비
...
**Imagine**(상상하다): Engram 위에서 spreading activation을 수행한다.

1. Engram → `neuron_map` + `synapse_map`(인접 리스트)
2. `start`에서 flux=1로 출발
3. 매 epoch: outgoing synapses에 `Impulse` 적용 → flux 전파 → `creativity`만큼 노이즈 가산
4. 전체 epoch의 flux를 누적 → flux 내림차순 정렬 → `Imagination` 반환

`Impulse`는 시냅스마다 가중치를 계산하는 함수다. decay, 균등, 커스텀 — 전략은 외부에서 주입한다. `creativity`는 **SNR 조절 노브**다: `0`이면 `Impulse`만으로 deterministic하게 수렴한다. `> 0`이면 노이즈를 더해 약한 연결도 활성화될 수 있다 — 발산적 연상, 깊은 수면의 뇌. `< 0`이면 노이즈를 빼 강한 연결만 살아남고 약한 것은 억제된다 — 수렴적 연상, 각성한 뇌. `start`가 Engram에 없으면 빈 `Imagination`을 반환한다.
